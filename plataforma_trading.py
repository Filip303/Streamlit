...
if df is not None and not df.empty:
    ...
    with col2:
        ...
        with col_a:
            st.metric("Último Precio", f"${df['Close'][-1]:.2f}",
                     f"{((df['Close'][-1] - df['Close'][-2])/df['Close'][-2]*100):.2f}%")
            st.metric("Volumen", f"{df['Volume'][-1]:,.0f}")

        with col_b:
            st.metric("Apertura", f"${df['Open'][-1]:.2f}")
            st.metric("Máximo", f"${df['High'][-1]:.2f}")
        ...
        price = st.number_input("Precio", min_value=0.01, value=float(df['Close'][-1]))
